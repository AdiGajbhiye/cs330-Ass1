...
TimerInterruptHandler(int dummy)
{
	...
	if(waitingList.IsEmpty()) return;	
    	NachOSThread *thread = (NachOSThread *)waitingList.SortedRemove(&waitTime);
	if(stats->totalTicks < waitTime){
	    waitingList.SortedInsert((void *)thread,waitTime);
	    return;
	}
	else{
	    scheduler->ThreadIsReadyToRun(thread);
	    if(waitingList.IsEmpty()) return;	
    	    thread = (NachOSThread *)waitingList.SortedRemove(&waitTime);
	    while(!waitingList.IsEmpty() && (stats->totalTicks >= waitTime)){
